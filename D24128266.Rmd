...
getwd()
#We are using a .sav file created from the SPSS file survey.sav  taken from SPSS Survival Manual 6th Edition Julie Pallant (http://spss.allenandunwin.com.s3-website-ap-southeast-2.amazonaws.com/data-files.html#.Wb0vvnWP-po). 
...
varsint<-c("metascore", "gross", "release_year","duration", "imdb_rating","votes" )
sdatasubset<-imdb[varsint]#create a subset of data with just the variables of interest
```

```{r missing, fig.cap="<b>Figure 1: Inspection of Missing Data</b>", include=TRUE, echo=FALSE}
summary(VIM::aggr(sdatasubset, sortVar=TRUE, combined=FALSE, only.miss=T, numbers=TRUE, cex.axis = 0.8))
```
...
```{r hist-metascore, include=FALSE, warning=FALSE}
#Create a histogram
gg_metascore <- ggplot(imdb, aes(x=metascore)) 
#Change the label of the x axis
gg_metascore <- gg_metascore + labs(x="metascore")
#manage binwidth and colours
gg_metascore <- gg_metascore + geom_histogram(binwidth=2, colour="black", aes(y=..density.., fill=..count..))
gg_metascore <- gg_metascore + scale_fill_gradient("Count", low="#DCDCDC", high="#7C7C7C")
#A a density curve using a normal denisty function
#Use stat_function to compute a normalised score for each value of metascore
#Pass the mean and standard deviation to this as parameters
#use the na.rm parameter to say how missing values are handled
gg_metascore <- gg_metascore + stat_function(fun=dnorm, color="red",args=list(mean=mean(imdb$metascore, na.rm=TRUE), sd=sd(imdb$metascore, na.rm=TRUE)))
...
metascore_skew<-semTools::skew(imdb$metascore)
metascore_kurt<-semTools::kurtosis(imdb$metascore)
metascore_skew[1]/metascore_skew[2]
metascore_kurt[1]/metascore_kurt[2]
...
zmetascore<- abs(scale(imdb$metascore))

FSA::perc(as.numeric(zmetascore), 1.96, "gt")
FSA::perc(as.numeric(zmetascore), 3.29, "gt")
...
Metascore scores were assessed for normality. Visual inspection of the histogram and QQ-Plot (see Figure 2 and Figure 3) identified some issues with skewness and kurtosis. Neither the standardized score for kurtosis (`r round(metascore_kurt[1]/metascore_kurt[2],2)`) nor the standardized score for skewness (`r round(metascore_skew[1]/metascore_skew[2],2)`) can be considered acceptable using the criteria proposed by Curran, West and Finch (1996). However as over 99% of standardized scores fall within the bounds of +/- 3.29, using the guidance of Field, Miles and Field (2013), the data can be considered to approximate a normal distribution (m=`r round(mean(imdb$metascore, na.rm=TRUE),2)`, sd=`r round(sd(imdb$metascore, na.rm=TRUE),2)`, n=`r length(imdb$metascore)-sum(is.na(imdb$metascore))`).
...
```{r hist-imdb-rating, include=FALSE}
#Create histogram
gs_rating <- ggplot(imdb, aes(x=imdb_rating)) + labs(x="imdb rating") + geom_histogram(binwidth=.2, colour="black", aes(y=..density.., fill=..count..)) + scale_fill_gradient("Count", low="#DCDCDC", high="#7C7C7C") + stat_function(fun=dnorm, color="red",args=list(mean=mean(imdb$imdb_rating, na.rm=TRUE), sd=sd(imdb$imdb_rating, na.rm=TRUE)))
...
rating_skew<-semTools::skew(imdb$imdb_rating)
rating_kurt<-semTools::kurtosis(imdb$imdb_rating)
rating_skew[1]/rating_skew[2]
rating_kurt[1]/rating_kurt[2]

#Calculate the percentage of standardized scores outside acceptable limits
zrating<- abs(scale(imdb$imdb_rating))

FSA::perc(as.numeric(zrating), 1.96, "gt")
FSA::perc(as.numeric(zrating), 3.29, "gt")
# Below we are reporting the normality assessment - embedding code within a paragraph to get the statistics generated ( you can just write it out)
...
Imdb rating scores were assessed for normality. Visual inspection of the histogram and QQ-Plot (see Figure 4 and Figure 5) identified some issues with skewness and kurtosis. While the standardized score for kurtosis (`r round(rating_kurt[1]/rating_kurt[2],2)`) could be considered acceptable using the criteria proposed by Curran, West and Finch (1996), but the standardized score for skewness (`r round(rating_skew[1]/rating_skew[2],2)`) was outside the acceptable range. However as over 99% of standardized scores fall within the bounds of +/- 3.29, using the guidance of Field, Miles and Field (2012), the data can be considered to approximate a normal distribution (m=`r round(mean(imdb$imdb_rating, na.rm=TRUE),2)`, sd=`r round(sd(imdb$imdb_rating, na.rm=TRUE),2)`, n=`r length(imdb$imdb_rating)-sum(is.na(imdb$imdb_rating))`).
...
#Create histogram
gs_votes <- ggplot(imdb, aes(x=votes)) + labs(x="votes") + geom_histogram(binwidth=150000, colour="black", aes(y=..density.., fill=..count..)) + scale_fill_gradient("Count", low="#DCDCDC", high="#7C7C7C") + stat_function(fun=dnorm, color="red",args=list(mean=mean(imdb$votes, na.rm=TRUE), sd=sd(imdb$votes, na.rm=TRUE)))
...
#Generate standardized scores for skewness and kurtosis
votes_skew<-semTools::skew(imdb$votes)
votes_kurt<-semTools::kurtosis(imdb$votes)
votes_skew[1]/votes_skew[2]
votes_kurt[1]/votes_kurt[2]

#Calculate the percentage of standardized scores outside acceptable limits
zvotes<- abs(scale(imdb$votes))

perc196<-FSA::perc(as.numeric(zvotes), 1.96, "gt")
# Below we are reporting the normality assessment - embedding code within a paragraph to get the statistics generated ( you can just write it out)
...
Film votes scores were assessed for normality. Visual inspection of the histogram and QQ-Plot (see Figure 7 and Figure 8) identified some issues with skewness and kurtosis. Neither the standardized score for kurtosis (`r round(votes_kurt[1]/votes_kurt[2],2)`) nor for skewness (`r round(votes_skew[1]/votes_skew[2],2)`) could be considered within the acceptable range using the criteria proposed by Curran, West and Finch (1996). However, as over 95% of standardized scores fall within the bounds of +/- 1.96 (as perc over 1.96 `r perc196`% is above), using the guidance of Field, Miles and Field (2012), the data can be considered to approximate a normal distribution (m=`r round(mean(imdb$votes, na.rm=TRUE),2)`, sd=`r round(sd(imdb$votes, na.rm=TRUE),2)`, n=`r length(imdb$votes)-sum(is.na(imdb$votes))`).
...
comedy_skew<-semTools::skew(comedies$imdb_rating)
comedy_kurt<-semTools::kurtosis(comedies$imdb_rating)
comedy_skew[1]/comedy_skew[2]
comedy_kurt[1]/comedy_kurt[2]

non_comedies<- filter(imdb, genre_comedy==FALSE)

non_comedy_skew<-semTools::skew(non_comedies$imdb_rating)
non_comedy_kurt<-semTools::kurtosis(non_comedies$imdb_rating)
non_comedy_skew[1]/non_comedy_skew[2]
non_comedy_kurt[1]/non_comedy_kurt[2]

zcomedy_rating<- abs(scale(comedies$imdb_rating))

comedy_perc196<-FSA::perc(as.numeric(zcomedy_rating), 1.96, "gt")

znon_comedy_rating<- abs(scale(non_comedies$imdb_rating))

non_comedy_perc196<-FSA::perc(as.numeric(znon_comedy_rating), 1.96, "gt")

```

<p style="text-align:justify">

The two groups (comedy/non comedy) of imdb ratings were assessed for normality. Visual inspection of the histogram identified some issues with skewness and kurtosis. However, as over 95% of standardized scores fall within the bounds of +/- 1.96 (as percentages over were calculated `r comedy_perc196`% and `r non_comedy_perc196`%). Therefore, using the guidance of Field, Miles and Field (2012), both groups can be considered to approximate the normal distribution with the comedy group distribution attributes as (m=`r round(mean(comedies$imdb_rating, na.rm=TRUE),2)`, sd=`r round(sd(comedies$imdb_rating, na.rm=TRUE),2)`, n=`r length(comedies$imdb_rating)-sum(is.na(comedies$imdb_rating))`) and the non comedy group ones as (m=`r round(mean(non_comedies$imdb_rating, na.rm=TRUE),2)`, sd=`r round(sd(non_comedies$imdb_rating, na.rm=TRUE),2)`, n=`r length(non_comedies$imdb_rating)-sum(is.na(non_comedies$imdb_rating))`).
...
No missing data was found for decades. The proportions of films ratings for each decade are shown in Figure 12.
...
```{r fig.cap="<b>Figure 12: Proportions of film ratings for each decade</b>" , echo=FALSE}
print(pie_decades)
```
...
The variable release_decade has been assessed in 2.4.1. The proportion of films in each release decade is shown in Figure 12.
